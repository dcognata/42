...
/*   Updated: 2018/09/20 11:35:37 by dcognata         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */

#include "libft.h"

static int	write_prec(t_pf_env *e, long long int nb, int lng)
{
	if (nb < 0)
		printf_print_less(e);
...
static void	wr_justify(t_pf_env *e, long long int nb, int lng)
{
	char	c;
	int		tmp;
...
static int	wl_justify(t_pf_env *e, long long int nb)
{
	char	c;
...
static void	write_nb(t_pf_env *e, long long int nb, int lng)
{
	if (e->precision.p < 0 && e->convert.flags[SHOW_SIGN] == 1 && nb >= 0
		&& e->width.bz != 1)
		printf_print_more(e);
