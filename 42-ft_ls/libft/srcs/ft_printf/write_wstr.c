...
/*   Updated: 2018/09/20 11:35:10 by dcognata         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */

#include "libft.h"

static int	wr_justify(t_pf_env *e, int lng)
{
	char	c;
...
static int	wl_justify(t_pf_env *e)
{
	while (e->width.w--)
	{
		e->written++;
...
static void	write_wc(t_pf_env *e, int c)
{
	if ((int)c < 0x80)
		ft_putwchar(c, e);
	else if (c < 0x800)
...
static int	print_null(t_pf_env *e, wchar_t *str)
{
	if (str == NULL)
	{
		ft_putstr("(null)");
